...
def model_fn(model_dir):
    ...
    """
    print("in model fn trying to create model")
    drowsy_model = YOLO(f"{model_dir}/drowsy_frozen.pt")
    print("drowsy model created")
...
def predict_fn(input_data, models):
    ...
    results_drowsy = drowsy_model.predict(input_data, conf=0.50)[0] # trying out different format
    #results_texting = texting_model.predict(input_data, conf=0.50)[0]
    ...
    print("extraction okay")
    all_boxes = drowsy_boxes

    # beep logic (same as before but cleaner)
    beep = len(all_boxes) > 0
    print("exiting predict fn")
    return {
        "beep": beep,
...
def output_fn(prediction, content_type='application/json'):
    ...
    """
    print("dumping to json in output fn")
